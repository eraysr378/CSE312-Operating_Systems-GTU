...
uint32_t KeyboardDriver::HandleInterrupt(uint32_t esp)
{
    ...
    if(key < 0x80)
    {
        ...
        case 0x32:
            ...
            CanRun::canRun = false;
            if (InterruptManager::timerInterruptCount == 5)
            {
            ...
            Process *collatz;
            collatz = interruptManager->processManager->GetProcessByPid(1);
            // collatz is terminated no need to schedule
            if (collatz->GetState() == Process::Terminated)
            {
                esp = (uint32_t)interruptManager->processManager->Schedule((CPUState *)esp);
                break;
            }
            // Adjust dynamic scheduling
            if (collatz->GetState() == Process::Running)
            {
                interruptManager->processManager->collatzRunCount++;
            }
            ...
            if (interruptManager->processManager->collatzRunCount >= 5)
            {
                collatz->SetPriority(110);
                // reset count
                interruptManager->processManager->collatzRunCount = 0;
            }
            // if collatz has been running for some time then decrease its priority number (increase its priority)
            else if (interruptManager->processManager->collatzRunCount <= -5)
            { 
                collatz->SetPriority(90);
                // remove it from queue
                interruptManager->processManager->readyQueue.removePid(collatz->GetPid());
                // and add to queue again in this way it will be the first in the queue because of its priority
                interruptManager->processManager->readyQueue.enqueue(collatz);
                // reset count
                interruptManager->processManager->collatzRunCount = 0;
            }
            esp = (uint32_t)interruptManager->processManager->Schedule((CPUState *)esp);
